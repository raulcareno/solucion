 ...
 * @author GEOVANNY
 */
public class ReporteMatriculadosDataSource implements JRDataSource{
    private Iterator itrAlumnos;
     private Iterator itrNodos;
   private Object valorAtual;
   private boolean irParaProximoAlumno = true;

    public ReporteMatriculadosDataSource(List lista) {
        super();
    ...
    Permisos permiso = new Permisos();
        ConteoMatriculas  nodo = (ConteoMatriculas) valorAtual;
        String fieldName = campo.getName();

       try {


        if ("nombrecurso".equals(fieldName)){
            valor = nodo.getNombrecurso();
        }else if ("especialidad".equals(fieldName)) {
            valor = nodo.getEspecialidad();
        }else if ("genero".equals(fieldName)) {
            valor = nodo.getGenero();
        } else if ("conteo".equals(fieldName)) {
            valor = nodo.getConteo();
