...
//                    mprofesor = (MateriaProfesor) adm.buscarClave((Integer) dos, MateriaProfesor.class);
    ...
    public List<Matriculas> cuadroverificarCompleto(Cursos curso, Sistemacalificacion sistema, Matriculas matriculaActual) {
//     int tamanio=0;
        Administrador adm = new Administrador();
            ...
            for (int j = 0; j < vec.size(); j++) {
                ...
                if (j == (vec.size() - 1)) {
                    ...
                    for (int sis = 0; sis <= sistemas.size() - 1; sis++) {
                        ...
                        try {
                            if (((Sistemacalificacion) sistemas.get(sis)).getPromediofinal().equals("SM")) {
                                inter.eval("Object sm =" + dos + ";");
                            } else if (((Sistemacalificacion) sistemas.get(sis)).getPromediofinal().equals("PG")) {
                                inter.eval("Object pg =" + val + ";");
                                pg = (val >= 7 ? "A" : (val < 5 ? "RM" : val <= 0 ? "PE" : "SP"));
                            } else if (((Sistemacalificacion) sistemas.get(sis)).getPromediofinal().equals("SU")) {
                                inter.eval("Object sp =" + dos + ";");
                                sp = (pg == "SP" ? (val >= 7 ? "A" : val <= 0 ? "SP" : "RM") : pg); //??
                            } else if (((Sistemacalificacion) sistemas.get(sis)).getPromediofinal().equals("RE")) {
                                inter.eval("Object rm =" + dos + ";");
                                rm = (pg == "RM" || sp == "RM" ? (val >= 7 ? "A" : val <= 0 ? "RM" : "GR") : pg); //??
                            } else if (((Sistemacalificacion) sistemas.get(sis)).getPromediofinal().equals("GR")) { //fin remedial
                                inter.eval("Object gr =" + dos + ";");
                                gr = (pg == "GR" || rm == "GR" || sp == "GR" ? (val >= 7 ? "A" : val <= 0 ? "GR" : "R") : pg); //??
                            } else if (((Sistemacalificacion) sistemas.get(sis)).getPromediofinal().equals("PF")) {
                                inter.eval("Object pf =" + dos + ";");
                                obs = "";
                                if (pg == "A") {
                                    obs = "aprobado";
                                } else if (pg == "SP" && sp == "A") {
...
//     int tamanio=0;
        ...
        List<Notanotas> notaFinal = adm.query("Select o from Notanotas as o "
                + "where o.sistema.periodo.codigoper = '" + periodo.getCodigoper() + "' and o.sistema.promediofinal = 'PF' ");
        if (notaFinal.size() <= 0) {
            try {
                Messagebox.show("No ha parametrizado el Promedio Final o General en Aportes...!", "Administrador Educativo", Messagebox.CANCEL, Messagebox.EXCLAMATION);
                return null;
        ...
        List<Notanotas> notaSupletorio = adm.query("Select o from Notanotas as o "
        ...
        String nSup = notaSupletorio.get(0).getNota();


        List<Notanotas> notaRemedial = adm.query("Select o from Notanotas as o "
        ...
        String nRem = notaRemedial.get(0).getNota();
...
//sd.getTrimestre().getCodigotrim();
        ...
        String q = "Select codigomap, mat.codigomat,notas.materia, " + query + ", 'obs'  from notas, materia_profesor , matriculas mat, estudiantes est "
                ...
                + " and notas.disciplina = false and materia_profesor.seimprime = true  "
                + "and matricula in (select codigomat from matriculas where  curso  =  '" + curso.getCodigocur() + "'  ) "
                + " and notas.materia >0 order by  CONCAT(est.apellido,' ',est.nombre), materia_profesor.orden";
...
//                    } else 
                    if (nota.getSistema().getPromediofinal().equals("PG")) {
                        if (validaConPromedioGeneral) {
                            List notasListRem = adm.queryNativo("Select  CAST(AVG(o." + nRem + ")as decimal (9,3)) from Notas as o"
                                    + " where o.materia =  " + materiaNo.getCodigo() + " and  "
                                    + " o.matricula = '" + matriculaNo.getCodigomat() + "' ");
                            BigDecimal nRemNota = (BigDecimal) ((Vector) notasListRem.get(0)).get(0);
                            List notasListSup = adm.queryNativo("Select  CAST(AVG(o." + nSup + ")as decimal (9,3)) from Notas as o"
                                    + " where o.materia =  " + materiaNo.getCodigo() + " and  "
                                    + " o.matricula = '" + matriculaNo.getCodigomat() + "' ");
                            BigDecimal nSupNota = (BigDecimal) ((Vector) notasListSup.get(0)).get(0);

                            if (val >= 7) {
                                obs = "";
                            } else if (val < 7 && val >= 5) {
                                obs = "SP";
                                if (nSupNota.doubleValue() >= 7) {
                                    obs = "";
                                } else if (nSupNota.doubleValue() == 0) {
                                    obs = "SP";
                                } else if (nSupNota.doubleValue() > 0) {
                                    obs = "RM";
                                    if (nRemNota.doubleValue() >= 7) {
                                        obs = "";
                                    } else if (nRemNota.doubleValue() > 0 && nRemNota.doubleValue() < 7) {
                                        obs = "GR";//falta validar si hay examen de gracia
                                    } else {
                                        obs = "RM";//falta validar si hay examen de gracia
                                    }
                                }
...
//                    }
                    ...
                    disciplina = equivalencia(disciplina, equivalenciasDisc);
                    //devolverNombreDisciplina(equivalenciasDisc, disciplina.doubleValue()).getNombre());
...
//                    DecimalFormat decimalFormat = new DecimalFormat(s);
    ...
    public JRDataSource cuadrofinalsupletorioMSGR(Cursos curso, Sistemacalificacion sistema, Double desde, Double hasta, Boolean soloSupletoriados) {
//     int tamanio=0;
        Administrador adm = new Administrador();
        Session ses = Sessions.getCurrent();
        Periodo periodo = (Periodo) ses.getAttribute("periodo");
        List sistemas = adm.query("Select o from Sistemacalificacion as o "
                + "where o.periodo.codigoper = '" + periodo.getCodigoper() + "' "
                + "and o.promediofinal in ('PG','SU','RE','GR','PF')  "
                + "   order by o.orden ");

        List<Notanotas> notas = adm.query("Select o from Notanotas as o "
                + "where  o.sistema.promediofinal in ('PG','SU','RE','GR','PF')    "
                + "and o.sistema.periodo.codigoper = '" + periodo.getCodigoper() + "'  "
                + " order by o.sistema.orden ");
        if (notas.size() <= 0) {
            try {
...
//                    + " order by o.sistema.orden ");
        ...
        int NoMateriasRemedial = 0;
        int NoMateriasGracia = 0;
        int NoMateriasSinNotas = 0;
        int ksis = 0;
        int primero = 0;
        for (Iterator itna = nativo.iterator(); itna.hasNext();) {
            Vector vec = (Vector) itna.next();
            ...
            mprofesor = (MateriaProfesor) adm.buscarClave((Integer) vec.get(0), MateriaProfesor.class);;
            matriculaNo = (Matriculas) adm.buscarClave((Integer) vec.get(1), Matriculas.class);
            ...
            if (!matriculaNo.toString().equals(matricula) && !matricula.equals("")) {
                NoMateriasRemedial = 0;
                NoMateriasGracia = 0;

                NoMateriasSinNotas = 0;
                ksis = 0;
                cont++;
            }
            matricula = matriculaNo.toString();

            materiaNo = (Global) adm.buscarClave((Integer) vec.get(2), Global.class);
            Double proGen = new Double((vec.get(3) + "") == (null) ? "0" : vec.get(3) + "");
            Double proSup = new Double((vec.get(4) + "") == (null) ? "0" : vec.get(4) + "");
            Double proRem = new Double((vec.get(5) + "") == (null) ? "0" : vec.get(5) + "");
            Double proGra = new Double((vec.get(6) + "") == (null) ? "0" : vec.get(6) + "");
            Double proFi = new Double((vec.get(7) + "") == (null) ? "0" : vec.get(7) + "");

            Trimestres t = new Trimestres(1);
            t.setDescripcion("");
            ...
            } else if (proGen < 7 && proGen >= 5 && proSup <= 0) {
                NoMateriasSinNotas++;
            } else if (proGen < 7 && proGen >= 5 && proSup < 7) {
                NoMateriasRemedial++;
            } else if (proGen < 5) {
                NoMateriasGracia++;
            }
            for (int i = 0; i < 3; i++) {
                ...
                if (NoMateriasRemedial > 0) {
                    nota.setObservacion("REMEDIAL");
                } else if (NoMateriasGracia > 0 && NoMateriasGracia <= 1 && NoMateriasSinNotas <= 0) {
                    nota.setObservacion("GRACIA");
                } else if (NoMateriasSinNotas > 0) {
                    nota.setObservacion("SIN NOTAS");
                } else if (NoMateriasGracia > 1) {
                    nota.setObservacion("SIN NOTAS");
                } else {
                    nota.setObservacion("APROBADO");
                }
                if (matriculaNo.getEstado().contains("Retirado")) {
                    nota.setObservacion("RETIRADO");
                }

                Sistemacalificacion sis = new Sistemacalificacion(i);

                if (i == 0) {
                    sis.setAbreviatura("PA");
                    sis.setOrden(1);
                    if (truncarNotas) {
                        nota.setNota(truncar(proGen, 2));
                    } else {
                        nota.setNota(redondear(proGen, 2));
                ...
                if (i == 1) {
                    sis.setAbreviatura("MSGR");
                    sis.setOrden(2);
                    if (truncarNotas) {
                        nota.setNota(truncar(proSup, 2));
                    } else {
                        nota.setNota(redondear(proSup, 2));

                    }

                }
                if (i == 2) {
                    sis.setAbreviatura("PF");
                    sis.setOrden(3);
                    if ((nota.getObservacion().contains("REMEDIAL") || nota.getObservacion().contains("GRACIA")
                            || nota.getObservacion().contains("SIN ")) && proSup > 0 && proSup < 7) {
                        nota.setNota("");
                    } else {
                        if (truncarNotas) {
                            nota.setNota(truncar(proFi, 2));
                        } else {
                            nota.setNota(redondear(proFi, 2));
                ...
                nota.setDisciplina(disciplina);
                if ((nota.getObservacion().contains("REMEDIAL") || nota.getObservacion().contains("GRACIA")
                        || nota.getObservacion().contains("SIN "))) {
                    nota.setAprovechamiento(null);
                    //nota.setDisciplina(disciplina);
                }
                if (matriculaNo.getEstado().contains("Retirado")) {
                    nota.setAprovechamiento(null);
...
//        }
    ...
    public JRDataSource cuadrofinalremedialesMSGR(Cursos curso, Sistemacalificacion sistema, Double desde, Double hasta, Boolean soloSupletoriados) {
//     int tamanio=0;
        Administrador adm = new Administrador();
        Session ses = Sessions.getCurrent();
        Periodo periodo = (Periodo) ses.getAttribute("periodo");
 ...
 Integer remedial = 5;
   ...
   List<MateriaProfesor> materiaProfesor = adm.query("Select o from MateriaProfesor as o "
            ...
            //SI ES QUE LA NOTA GENERAL ES MEJOR A 5 OSEA EL DE GRACIA, O A SU VEZ LA NOTA DE SUPLETORIO SEA MENOR A 7 Y SUPLETORIO >0 
            String qRm = "Select DISTINCT o.matricula.codigomat from Notas as o"
                    + " where (o." + nGeneral + " < " + remedial + " and o." + nGeneral + " < " + 7 + " "
                    + "or ( o." + nSup + " < " + 7 + " "
//                    + " AND  o." + nSup + " > " + 0 + " "
                    + ") ) AND o." + nGeneral + " < 7  and o.materia.codigo in (" + codigoMapProfesor + ") "
                    + " and o.matricula.curso.codigocur = '" + curso.getCodigocur() + "' "
                    + "  and o.matricula.estado in ('Matriculado','Retirado') ";
            List matriculasList = adm.query(qRm);
...
//        }
        ...
        String q = "Select codigomap, mat.codigomat,notas.materia, " + query + "   from notas, materia_profesor , matriculas mat, estudiantes est "
                ...
                + "and materia_profesor.curso = '" + curso.getCodigocur() + "' "
                + " and notas.disciplina = false and materia_profesor.seimprime = true  "
                + "and matricula in ( " +matriculasSupletoriadas + "  ) "
                + " and notas.materia >0 order by  CONCAT(est.apellido,' ',est.nombre), materia_profesor.orden";
        ...
        for (Iterator itna = nativo.iterator(); itna.hasNext();) {
            ...
            } else if (proGen < 7 && proGen >= 5 && proRem <= 0) {
                NoMateriasSinNotas++;
            } else if (proGen < 7 && proGen >= 5 && proRem < 7) {
                NoMateriasGracia++;
            } else if (proGen < 5 && proSup<7 && proSup>0) {
                NoMateriasGracia++;
            }
            for (int i = 0; i < 3; i++) {
                Nota nota = new Nota();
                ...
                if (i == 1) {
                    ...
                    if (truncarNotas) {
                        nota.setNota(truncar(notaS, 2));
                    } else {
                        nota.setNota(redondear(notaS, 2));
...
//            }
        ...
        ArrayList<Matriculas> listaMatriculas = new ArrayList<Matriculas>();
        List<Nota> lisNotasArreglado = new ArrayList();
        if (soloSupletoriados==false) {
            for (Iterator<Nota> it = lisNotas.iterator(); it.hasNext();) {
                Nota nota = it.next();
                 
                    if (nota.getObservacion().contains("SIN ") || nota.getObservacion().contains("REMEDIAL") || nota.getObservacion().contains("GRACIA") ||  nota.getObservacion().contains("RETIR")) {
                        System.out.println("" + nota.getMatricula() + " " + nota.getMateria() + " " + nota.getNota());
                        listaMatriculas.add(nota.getMatricula());
                    }
   ...
   public JRDataSource cuadrofinalgraciaMSGR(Cursos curso, Sistemacalificacion sistema, Double desde, Double hasta, Boolean soloSupletoriados) {
//     int tamanio=0;
        Administrador adm = new Administrador();
        Session ses = Sessions.getCurrent();
        Periodo periodo = (Periodo) ses.getAttribute("periodo");
 ...
 Integer remedial = 5;
   ...
   List<MateriaProfesor> materiaProfesor = adm.query("Select o from MateriaProfesor as o "
            ...
            //SI ES QUE LA NOTA GENERAL ES MEJOR A 5 OSEA EL DE GRACIA, O A SU VEZ LA NOTA DE SUPLETORIO SEA MENOR A 7 Y SUPLETORIO >0 
            String qRm = "Select DISTINCT o.matricula.codigomat from Notas as o"
                    + " where (o." + nGeneral + " < " + 7 + " and o." + nRem + " < " + 7 + " "
                    + " and  o." + nSup + " < " + 7 + "  "
                    + ") and o.materia.codigo in (" + codigoMapProfesor + ") "
                    + " and o.matricula.curso.codigocur = '" + curso.getCodigocur() + "' "
                    + "  and o.matricula.estado in ('Matriculado','Retirado') ";
//            
            
//            String qRm = "SELECT o.matricula FROM Notas o  "
//                    + " WHERE  (o." + nSup + " < " + 7 + " and o." + nRem + " < " + 7 + "  and o." + nGeneral + " < " + 7 + " )   "
//                    + " AND o.materia in (" + codigoMapProfesor + ")  "
//                    + " AND m.curso = '" + curso.getCodigocur() + "'  "
//                    + "   AND m.estado IN ('Matriculado')"
...
//        }
        ...
        for (Iterator itna = nativo.iterator(); itna.hasNext();) {
            ...
            } else if (proGen < 7 && proGen >= 5 && proGra <= 0) {
                NoMateriasSinNotas++;
            } else if (proGen < 7 && proGen >= 5 && proGra < 7) {
                NoMateriasGracia++;
            } else if (proGen < 5 && proSup<7 && proSup>0) {
                NoMateriasGracia++;
            }
...
//            }
    ...
    public JRDataSource cuadrofinalremediales(Cursos curso, Sistemacalificacion sistema, Double desde, Double hasta, Boolean soloSupletoriados) {
//     int tamanio=0;
        Administrador adm = new Administrador();
        Session ses = Sessions.getCurrent();
        Periodo periodo = (Periodo) ses.getAttribute("periodo");
...
//                    DecimalFormat decimalFormat = new DecimalFormat(s);
    ...
    public JRDataSource cuadrofinalgracia(Cursos curso, Sistemacalificacion sistema, Double desde, Double hasta, Boolean soloSupletoriados) {
//     int tamanio=0;
        Administrador adm = new Administrador();
        Session ses = Sessions.getCurrent();
        Periodo periodo = (Periodo) ses.getAttribute("periodo");
        ...
        List<Notanotas> notaGracia = adm.query("Select o from Notanotas as o "
                + "where o.sistema.periodo.codigoper = '" + periodo.getCodigoper() + "' and o.sistema.promediofinal = 'GR' ");
        if (notaRemedial.size() <= 0) {
            try {
                Messagebox.show("No ha parametrizado el Promedio General en Aportes...!", "Administrador Educativo", Messagebox.CANCEL, Messagebox.EXCLAMATION);
        ...
        String nGrac = notaGracia.get(0).getNota();
...
//                    val = redondear((Double) dos, 2);
                    ...
                    if (nota.getSistema().getPromediofinal().equals("PG")) {
                        if (validaConPromedioGeneral) {
                            ...
                            List notasListGr = adm.queryNativo("Select  CAST(AVG(o." + nGrac + ")as decimal (9,3)) from Notas as o"
                                    + " where o.materia =  " + materiaNo.getCodigo() + " and  "
                                    + " o.matricula = '" + matriculaNo.getCodigomat() + "' ");
                            BigDecimal nGrNota = (BigDecimal) ((Vector) notasListGr.get(0)).get(0);
                            ...
                            if (val >= 7) {
                                ...
                                } else if (nRemNota.doubleValue() > 0) {
//                                    obs = "RM";
                                    if (nRemNota.doubleValue() >= 7) {
                                        obs = "";
                                    } else if (nRemNota.doubleValue() < 7) {
                                        obs = "GR";//falta validar si hay examen de gracia
                                    } else {
                                        obs = "RM";
                                    }
                        ...
                        if (obs.equals("RM")) {
                            norm++;
                            if (!observacion.contains("Pierde")) {
                                //nota.setObservacion("Remedial");
                        ...
                        } else if (obs.equals("PA")) {

                            observacion = "Pierde el Año";
                            nota.setObservacion(observacion);
                        } else if (obs.equals("GR")) {
                            nogr++;
                            observacion = "Gracia";
                            nota.setObservacion(observacion);
                        }
        ...
        for (Iterator itna = nativo.iterator(); itna.hasNext();) {
            ...
            for (int j = 0; j < vec.size(); j++) {
                ...
                if (j >= 3) {
                    ...
                    //nota.setNota(val);
                    if (mprofesor.getCuantitativa() == false) {
                        String scala = mprofesor.getEscala();
                        try {
                            if (materiaNo.getCodigo().intValue() == 0) {
                                nota.setNota(equivalencia(dos, equivalenciasDisc));
                            } else {
                                //nota.setNota(equivalencia(dos, equivalencias));
                                nota.setNota(equivalencia(dos, (scala.equals("AP") ? equivalencias : scala.equals("DR") ? equivalenciasDisc : scala.equals("CL") ? equivalenciasCl : equivalencias)));
                            }
                        } catch (Exception e) {
                            System.out.println("NO HAY EQUIVALENCIAS DE DISCIPLINA..." + e);
                        }
...
//                    disciplina = aprovecha;
    ...
    public JRDataSource cuadrofinal(Cursos curso, Sistemacalificacion sistema, Double desde, Double hasta, Boolean incluir) {
//     int tamanio=0;
        Administrador adm = new Administrador();
        Session ses = Sessions.getCurrent();
        Periodo periodo = (Periodo) ses.getAttribute("periodo");
...
//sd.getTrimestre().getCodigotrim();
        ...
        String comple = " and notas.promedia = true  ";
        if (incluir) {
            comple = "";
        }
        String q = "Select codigomap, matricula,notas.materia, " + query + "  from notas, materia_profesor , matriculas mat, estudiantes est "
                + "where notas.materia =  materia_profesor.materia  AND notas.matricula = mat.codigomat AND est.codigoest = mat.estudiante "
                + "and materia_profesor.curso = '" + curso.getCodigocur() + "' "
                + comple + "and notas.disciplina = false and materia_profesor.seimprime = true  "
                + "and matricula in (select codigomat from matriculas where  curso  =  '" + curso.getCodigocur() + "'  ) "
                + "order by  CONCAT(est.apellido,' ',est.nombre), materia_profesor.orden";
...
//                System.out.println(""+q);
    ...
    //libreta dividida en optativas

    public JRDataSource libretasDivida(Cursos curso, Matriculas matri, Sistemacalificacion sistema) throws InterruptedException {
//     int tamanio=0; -2
        Administrador adm = new Administrador();
        Session ses = Sessions.getCurrent();
        Periodo periodo = (Periodo) ses.getAttribute("periodo");
...
//DECIMALESDIS
        ...
        List<Nota> lisNotas = new ArrayList();
        ArrayList lisNotasC = new ArrayList();
        ArrayList lisNotasComplementarias = new ArrayList();
        List<Matriculas> matriculas = new ArrayList();
        if (matri.getCodigomat().equals(-2)) {
            matriculas = adm.query("Select o from Matriculas as o where  o.estado in ('Matriculado','Recibir Pase')  and o.curso.codigocur = '" + curso.getCodigocur() + "' order by o.estudiante.apellido ");
...
//                System.out.println(""+q);
            ...
            //imprimo las optativas

            String qO = "Select matriculas.codigomat,notas.materia,notas.cuantitativa, " + query + "  from matriculas "
                    + "left join  estudiantes on matriculas.estudiante = estudiantes.codigoest  "
                    + "left join notas on matriculas.codigomat = notas.matricula  "
                    + "and notas.materia != 0  "
                    + "where matriculas.curso = '" + curso.getCodigocur() + "'  "
                    ...
                    + "and notas.materia != 0 "
                    + "order by estudiantes.apellido, notas.orden";
            System.out.println("NOTAS GENERALES optativas" + qO);
            List nativoO = adm.queryNativo(qO);
            //Nota nota = new Nota();

            for (Iterator itna = nativoO.iterator(); itna.hasNext();) {
                Vector vec = (Vector) itna.next();
                Matriculas matriculaNo = null;
                Boolean cuantitativa = false;
                Global mate = new Global();
...
//                        System.out.println(""+e);
                    ...
                    if (j >= 3) {
                        ...
                        coll.setSistema(((Sistemacalificacion) sistemas.get(ksis)).getAbreviatura());
                        coll.setTipo(((Sistemacalificacion) sistemas.get(ksis)).getTrimestre().getDescripcion());
                        lisNotasComplementarias.add(coll);
            ...
            nota.setMatricula(matriculas1);
            nota.setNotas(lisNotasC);
            nota.setNotasComplementarias(lisNotasComplementarias);
            nota.setFaltas(lisFaltas);
            nota.setAutoevaluacion(lisAutoevaluacion);
            lisNotas.add(nota);

            lisFaltas = new ArrayList();
            lisAutoevaluacion = new ArrayList();
            lisNotasC = new ArrayList();
            lisNotasComplementarias = new ArrayList();
        }
...
//        lisNotas = null;
    ...
    public boolean buscarPromediada(List<MateriaProfesor> materiaProfesores, Global materia) {
        for (Iterator<MateriaProfesor> it = materiaProfesores.iterator(); it.hasNext();) {

            MateriaProfesor materiaProfesor = it.next();
            if (materia.getCodigo().equals(materiaProfesor.getMateria().getCodigo())) {
                return materiaProfesor.getMinisterio();
            }
    ...
    public String buscarEscala(List<MateriaProfesor> materiaProfesores, Global materia) {
        for (Iterator<MateriaProfesor> it = materiaProfesores.iterator(); it.hasNext();) {

            MateriaProfesor materiaProfesor = it.next();
            if (materia.getCodigo().equals(materiaProfesor.getMateria().getCodigo())) {
                return materiaProfesor.getEscala();
            }
    ...
    public boolean buscarSeImprime(List<MateriaProfesor> materiaProfesores, Global materia) {
        for (Iterator<MateriaProfesor> it = materiaProfesores.iterator(); it.hasNext();) {

            MateriaProfesor materiaProfesor = it.next();
            if (materia.getCodigo().equals(materiaProfesor.getMateria().getCodigo())) {
                return materiaProfesor.getSeimprime();
            }
...
//DECIMALESDIS
        ...
        List<Nota> lisNotas = new ArrayList();
        ArrayList lisNotasC = new ArrayList();
        ArrayList lisNotasComple = new ArrayList();
        List<Matriculas> matriculas = new ArrayList();
        if (matri.getCodigomat().equals(-2)) {
            matriculas = adm.query("Select o from Matriculas as o where  o.estado in ('Matriculado','Recibir Pase')  and o.curso.codigocur = '" + curso.getCodigocur() + "' order by o.estudiante.apellido ");
        } else {
        ...
        int maxima = 100;
        int maximaMinisterio = 100;
        Long cantidadMateriasMinisterio = 0L;
        Long cantidadMateriasNoMinisterio = 0L;
        try {
            cantidadMateriasMinisterio = (Long) adm.querySimple("Select count(o) from MateriaProfesor as o "
                    + "where o.curso.codigocur = '" + curso.getCodigocur() + "' and o.ministerio = true and o.materia.codigo > 1 ");
            maxima = cantidadMateriasMinisterio.intValue() + 4;
            maximaMinisterio = cantidadMateriasMinisterio.intValue();
        } catch (Exception a) {
            a.printStackTrace();
        }
        try {
            cantidadMateriasNoMinisterio = (Long) adm.querySimple("Select count(o) from MateriaProfesor as o "
                    + "where o.curso.codigocur = '" + curso.getCodigocur() + "' and o.ministerio = false and o.materia.codigo >1 and o.seimprime = true  ");
            maxima = cantidadMateriasMinisterio.intValue() + 4;
            maximaMinisterio = cantidadMateriasMinisterio.intValue();
        } catch (Exception a) {
            a.printStackTrace();
        ...
        for (Matriculas matriculas1 : matriculas) {
            //Matriculas matriculas1 = itm.next();
            maxima = cantidadMateriasMinisterio.intValue() + 3;
...
//                        System.out.println(""+e);
                        ...
                        if (j >= 3) {
                            ...
                            if (cuantitativa == false || ((Sistemaevaluacion) sisEvaluaciones.get(ksis)).getEsequivalencia()) {
                                String scala = buscarEscala(materiaProfesores, mate);
                                coll2.setNota(equivalencia(dos, (scala.equals("AP") ? equivalencias : scala.equals("DR") ? equivalenciasDisc : scala.equals("CL") ? equivalenciasCl : equivalencias)));
                            } else {
//                                if (val == 0.0) {
//                                    coll2.setNota("");
//                                }
...
//                            if(mate.getDescripcion().contains("TUTO")){
//                                System.out.println("tutoria:   ");
//                            }
                            if ((buscarPromediada(materiaProfesores, mate))) {
                                lisNotasC.add(coll2);
                            } else {
                                ...
                                if (matActual.equals("")) {
                                    maxima++;
                                    matActual = mate.getCodigo() + "";
                                } else {
                                    if (!(mate.getCodigo() + "").equals(matActual)) {
                                        maxima++;
                                        matActual = mate.getCodigo() + "";
                                    }
                                }

//                                coll2.setOrden(maxima);
                                lisNotasC.add(coll2);
                                //maxima++;
                            }
            ...
            maxima = cantidadMateriasMinisterio.intValue() + 4;
            //maxima = cantidadMateriasMinisterio.intValue();
            Nota nota = new Nota();
            Object promedioFinal = null, disciplinaFinal = null;
            for (Iterator itna = nativo.iterator(); itna.hasNext();) {
                Vector vec = (Vector) itna.next();
...
//                                        System.out.println(""+e);
                        ...
                        if (j > 0) {
                            ...
                            promedioFinal = dos;
                            //coll.setOrden(maximaMinisterio+1);
                            coll.setOrden(100);
                            coll.setMateria(mate.getDescripcion());
                            coll.setMatricula("" + matriculas1.getCodigomat());
                            coll.setEstudiante(matriculas1.getEstudiante().getApellido() + " " + matriculas1.getEstudiante().getNombre());
...
//                                        System.out.println(""+e);
                        ...
                        if (j > 0) {
                            ...
                            disciplinaFinal = dos;
                            coll.setOrden(101);
                            //coll.setOrden(maximaMinisterio+2);
                            coll.setMateria(mate.getDescripcion());
                            coll.setMatricula("" + matriculas1.getCodigomat());
                            coll.setPlan("");
                            coll.setRecomendacion("");
            ...
            //COMPLEMENTARIAS
            if (cantidadMateriasNoMinisterio.intValue() > 100) {
//IMPRIMO EL PROMEDIO
                q = "Select matricula," + query2 + "  from notas "
                        + "where notas.matricula = '" + matriculas1.getCodigomat() + "' "
                        + "and notas.seimprime = true "
...
//                System.out.println("NOTAS DE promedio " + q);
                ...
                for (Iterator itna = nativo.iterator(); itna.hasNext();) {
                    ...
                    Global mate = new Global();
                    mate.setCodigo(0);
                    mate.setDescripcion("COMPLEMENTARIAS (N)");
                    int ksis = 0;
                    for (int j = 0; j < vec.size(); j++) {
                        Object dos = vec.get(j);
                        Double val = 0.0;
...
//                                        System.out.println(""+e);
                        ...
                        if (j > 0) {
                            ...
                            promedioFinal = dos;
                            coll.setOrden(maximaMinisterio + 3);
                            coll.setMateria(mate.getDescripcion());
                            coll.setMatricula("" + matriculas1.getCodigomat());
                            coll.setEstudiante(matriculas1.getEstudiante().getApellido() + " " + matriculas1.getEstudiante().getNombre());
                            coll.setSistema(((Sistemacalificacion) sistemas.get(ksis)).getAbreviatura());
...
//                System.out.println(""+q);
            ...
            nota.setNotas(lisNotasC);
//            nota.setNotasComplementarias(lisNotasComple); 
            nota.setFaltas(lisFaltas);
            nota.setAutoevaluacion(lisAutoevaluacion);
            lisNotas.add(nota);
...
//                        matriculaNo = (AcaMatricula) adm.buscarAcaMatricula((Integer) dos);
    ...
    public JRDataSource promocion4(Cursos curso, Matriculas matri, Boolean horizontal) { //CON DECIMALES
//     int tamanio=0; -2
        Administrador adm = new Administrador();
        ...
        List<Notanotas> notas = adm.query("Select o from Notanotas as o "
                + " where o.sistema.periodo.codigoper = '" + periodo.getCodigoper() + "'  "
                + "and o.sistema.promediofinal = 'PF' ");
        List<Notanotas> notasSelect = adm.query("Select o from Notanotas as o "
                + " where o.sistema.periodo.codigoper = '" + periodo.getCodigoper() + "'  "
                + "and o.sistema.promedioparcial = true order by o.sistema.orden ");
        List<Notanotas> notasPG = adm.query("Select o from Notanotas as o "
                + " where o.sistema.periodo.codigoper = '" + periodo.getCodigoper() + "'  "
                + "and o.sistema.promediofinal = 'PG' ");
        List<Trimestres> trimestres = adm.query("Select o from Trimestres as o where o.periodo = " + periodo.getCodigoper() + " ");
...
//        System.out.println(""+q);
            ...
            for (int i = 0; i < equivalenciasFaltasSoloDias.size(); i++) {
                query3 += "sum(disciplina.nota" + w + "),";
                w++;
            }
            query3 = query3.substring(0, query3.length() - 1);
            int t1 = 0;
            for (Iterator<Trimestres> it = trimestres.iterator(); it.hasNext();) {
                Trimestres trimestres1 = it.next();
                //IMPRIMO LAS FALTAS
                String qf = "Select " + query3 + "  from disciplina, sistemacalificacion "
                        + "where matricula = '" + matriculas1.getCodigomat() + "' "
                        + " and disciplina.sistema = sistemacalificacion.codigosis "
                        + "  "
                        + " and sistemacalificacion.trimestre = " + trimestres1.getCodigotrim() + " "
                        + " group by sistemacalificacion.trimestre ";
                ...
                for (Iterator itnaF = nativoF.iterator(); itnaF.hasNext();) {
                    ...
                    for (int jF = 0; jF < vecF.size(); jF++) {
                        Object dosF = vecF.get(jF);
                        Integer valF = new Integer(dosF.toString());
                        if ((equivalenciasFaltasSoloDias.get(jF)).getEsfi()) {
                            if (t1 == 0) {
                                q1f = valF;
                            } else if (t1 == 1) {
                                q2f = valF;
            ...
            //notasSelect;
            String preSelect = "";
            for (Iterator it = notasSelect.iterator(); it.hasNext();) {
                Notanotas object = (Notanotas) it.next();
                preSelect += object.getNota() + ",";
            }
            if (preSelect.length() > 0) {
                preSelect = preSelect.substring(0, preSelect.length() - 1);
            }
            q = "Select matriculas.codigomat,notas.materia,notas.cuantitativa, " + preSelect + "  "
                    + "from matriculas  "
                    + "left join estudiantes on matriculas.estudiante = estudiantes.codigoest "
                    + "left join notas on matriculas.codigomat = notas.matricula "
                    + "where matriculas.curso = '" + matriculas1.getCurso().getCodigocur() + "'  "
...
//                                }
                        ...
                        not.setDq1(new Double(q1f));
                        not.setDq2(new Double(q2f));
                        not.setDisciplina(disciplina);
                        if ((Double) dos >= matriculas1.getCurso().getAprobacion()) {
                            not.setEstadoMateria("APROBADO");
                        } else {
...
//  RANGOS
    ...
    public void validacionAprobado(List Notas, Matriculas mat) {
    }
...
//                + "where o.periodo.codigoper = '" + periodo.getCodigoper() + "' and o.grupo = 'AP'");
        ...
        try {
            remedial = regresaVariableParametrosDecimal("REMEDIAL", parametrosGlobales).intValue();
            if (remedial.equals(new Integer(0))) {
                remedial = 5;
            }
        } catch (Exception a) {
            remedial = 5;
        ...
        if (tipo.equals("SUP")) {
            ...
            //SI ES QUE LA NOTA GENERAL ES MEJOR A 5 OSEA EL DE GRACIA, O A SU VEZ LA NOTA DE SUPLETORIO SEA MENOR A 7 Y SUPLETORIO >0 
            String qRm = "Select DISTINCT o.matricula.codigomat from Notas as o"
                    + " where (o." + nGeneral + " < " + remedial + " and o." + nGeneral + " < " + 7 + " or ( o." + nSup + " < " + 7 + " AND  o." + nSup + " > " + 0 + " ) ) and o.materia.codigo in (" + codigoMapProfesor + ") "
                    + " and o.matricula.curso.codigocur = '" + curso.getCodigocur() + "' "
                    + "  and o.matricula.estado in ('Matriculado','Retirado') ";
            List matriculasList = adm.query(qRm);
            ...
            if (notaSupletorio.size() <= 0) {
                try {
                    Messagebox.show("No ha parametrizado el SUPLETORIO General en Aportes...!", "Administrador Educativo", Messagebox.CANCEL, Messagebox.EXCLAMATION);
            ...
            if (notaRemedial.size() <= 0) {
                try {
                    Messagebox.show("No ha parametrizado el examen remedial en Aportes...!", "Administrador Educativo", Messagebox.CANCEL, Messagebox.EXCLAMATION);
            ...
            /*
             *
             * SELECT COUNT(o.matricula), o.matricula,
             * o.materia,o.nota12,o.nota13 FROM Notas o, Matriculas m WHERE
             * (o.nota12 < 7 AND o.nota13 < 7 AND nota11< 7 ) AND o.materia IN
             * (145,74,86,103,201,79,237,194) AND m.curso = '425' AND m.estado
             * IN ('Matriculado','Retirado') AND m.codigomat = o.matricula GROUP
             * BY o.matricula SELECT COUNT(o.matricula), o.matricula,
             * o.materia,o.nota12,o.nota13 FROM Notas o, Matriculas m WHERE
             * (o.nota12 < 7 AND o.nota13 < 7 AND nota11< 7 ) AND o.materia IN
             * (145,74,86,103,201,79,237,194) AND m.curso = '425' AND m.estado
             * IN ('Matriculado','Retirado') AND m.codigomat = o.matricula GROUP
             * BY o.matricula HAVING COUNT(o.matricula) < 2
             *
             *
             *
             */
            ...
            //SI ES QUE LA NOTA GENERAL ES MEJOR A 5 OSEA EL DE GRACIA, O A SU VEZ LA NOTA DE SUPLETORIO SEA MENOR A 7 Y SUPLETORIO >0 
//            String qRm = "Select o.matricula.codigomat from Notas as o"
//                    + " where (o." + nSup + " < " + 7 + " and o." + nRem + " < " + 7 + "   ) and o.materia.codigo in (" + codigoMapProfesor + ") "
//                    + " and o.matricula.curso.codigocur = '" + curso.getCodigocur() + "' "
//                    + "  and o.matricula.estado in ('Matriculado','Retirado') ";
//            
            String qRm = "SELECT o.matricula FROM Notas o, Matriculas m "
                    + " WHERE  (o." + nSup + " < " + 7 + " and o." + nRem + " < " + 7 + "  and o." + nGeneral + " < " + 7 + " )   "
                    + " AND o.materia in (" + codigoMapProfesor + ")  "
                    + " AND m.curso = '" + curso.getCodigocur() + "'  "
            ...
            matriculasSupletoriadas = matriculasSupletoriadas.replace("[", "").replace("]", "");
            if (matriculasSupletoriadas.length() > 0) {
                matriculasSupletoriadas = matriculasSupletoriadas.substring(0, matriculasSupletoriadas.length() - 1);
            }
...
//        List<ParametrosGlobales> parametrosGlobales = adm.query("Select o from ParametrosGlobales as o "
        ...
        String q = "Select codigomap, matricula,notas.materia, " + query + "  from notas, materia_profesor , matriculas mat, estudiantes est "
                ...
                + "and notas.promedia = true and notas.disciplina = false and materia_profesor.seimprime = true  "
                + "and matricula in (select codigomat from matriculas where  curso  =  '" + curso.getCodigocur() + "' "
                + " and estado in ('Matriculado','Retirado')  ) "
                + " " + (tipo.equals("REM") || tipo.equals("GRA") || tipo.equals("SUP") ? " and mat.codigomat in (" + matriculasSupletoriadas + ") " : "")
                + "order by  CONCAT(est.apellido,' ',est.nombre), materia_profesor.orden";
...
//                    System.out.println("VALORCELDA:"+cellNota.getStringCellValue()+"");
                    ...
                    if (perdidos.contains(matriculaNo.getCodigomat())) {
                        if (tipo.equals("SUP")) {
                            cellObss.setCellValue("SP");
                        } else if (tipo.equals("REM")) {
                            cellObss.setCellValue("RM");
                        } else {
