...
public class ParametrosBean {
    ...
    Parametros object;
    Administrador adm;
    protected List<Parametros> model;
...
//                Logger.getLogger(ParametrosBean.class.getName()).log(Level.SEVERE, null, ex);
    ...
    public String editarAction(Parametros obj) {
        inicializar();

        object = obj;
        System.out.println("" + object.getIdParametros());
        return null;
    }

    protected void inicializar() {
        object = new Parametros(0);
        cargarDataModel();
    }
 ...
 if (object.getNombre().isEmpty()) {
        ...
        if (object.getIdParametros() == 0) {
            if (!permisos.verificarPermisoReporte("Parametros", "agregar_parametros.jspx", "agregar", true, "PARAMETROS")) {
            ...
            try {
                if (adm.existe("Parametros", "nombre", object.getNombre()).size() <= 0) {
                    object.setIdParametros(adm.getNuevaClave("Parametros", "idParametros"));
                    adm.guardar(object);
                    aud.auditar(adm,this.getClass().getSimpleName().replace("Bean", ""), "guardar", "", object.getIdParametros()+"");
                    inicializar();
            ...
            if (!permisos.verificarPermisoReporte("Parametros", "actualizar_parametros.jspx", "agregar", true, "PARAMETROS")) {
                FacesContext.getCurrentInstance().addMessage(findComponent(context.getViewRoot(), "form").getClientId(), new FacesMessage(FacesMessage.SEVERITY_ERROR, "No tiene permisos para realizar ésta acción", "No tiene permisos para realizar ésta acción"));                return null;
            }
            try {
                adm.actualizar(object);
                aud.auditar(adm,this.getClass().getSimpleName().replace("Bean", ""), "actualizar", "", object.getIdParametros()+"");
                ...
                java.util.logging.Logger.getLogger(ParametrosBean.class.getName()).log(Level.SEVERE, null, e);
                FacesContext.getCurrentInstance().addMessage(findComponent(context.getViewRoot(), "form").getClientId(), new FacesMessage(FacesMessage.SEVERITY_ERROR, e.getMessage(), e.getMessage()));
            }
     ...
     */
    public String eliminar(Parametros obj) {
        FacesContext context = FacesContext.getCurrentInstance();
        try {
            if (!permisos.verificarPermisoReporte("Parametros", "eliminar_parametros.jspx", "eliminar", true, "PARAMETROS")) {
                FacesContext.getCurrentInstance().addMessage(findComponent(context.getViewRoot(), "form").getClientId(), new FacesMessage(FacesMessage.SEVERITY_ERROR, "No tiene permisos para realizar ésta acción", "No tiene permisos para realizar ésta acción"));                return null;
            }
            adm.eliminarObjeto(Parametros.class, obj.getIdParametros());
            aud.auditar(adm,this.getClass().getSimpleName().replace("Bean", ""), "eliminar", "", obj.getIdParametros()+"");
            inicializar();
            cargarDataModel();
    ...
    public List<SelectItem> getSelectedItem() {
        try {
            List<Parametros> datos = adm.query("Select o from Parametros as o");
            List<SelectItem> items = new ArrayList<SelectItem>();
            for (Parametros obj : datos) {
    ...
    public void buscar() {
        try {
            //setModel(adm.listar("Parametros"));
            model = (adm.query("Select o from Parametros as o where o.nombre like '%" + textoBuscar + "%'"));
            setModel(model);
     ...
     * @return
     */
    public List<Parametros> getModel() {
        if (model == null) {
            cargarDataModel();
        }
        return model;
    ...
    public void setModel(List<Parametros> model) {
        this.model = model;
    }
    ...
    public void setObject(Parametros object) {
        this.object = object;
    }
}
