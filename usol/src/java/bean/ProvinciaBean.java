...
//        }
        ...
        if (!permisos.verificarPermisoReporte("Provincia", "ingresar_provincia.jspx", "ingresar", true, "PARAMETROS")) {
            try {
...
//                Logger.getLogger(ProvinciaBean.class.getName()).log(Level.SEVERE, null, ex);
    ...
    public String guardar() {
        ...
        if (object.getIdProvincia() == 0) {
            if (!permisos.verificarPermisoReporte("Provincia", "agregar_provincia.jspx", "agregar", true, "PARAMETROS")) {
            ...
            if (!permisos.verificarPermisoReporte("Provincia", "actualizar_provincia.jspx", "agregar", true, "PARAMETROS")) {
                FacesContext.getCurrentInstance().addMessage(findComponent(context.getViewRoot(), "form").getClientId(), new FacesMessage(FacesMessage.SEVERITY_ERROR, "No tiene permisos para realizar ésta acción", "No tiene permisos para realizar ésta acción"));                return null;
            }
            try {
    ...
    public String eliminar(Provincia obj) {
        FacesContext context = FacesContext.getCurrentInstance();
        try {
            if (!permisos.verificarPermisoReporte("Provincia", "eliminar_provincia.jspx", "eliminar", true, "PARAMETROS")) {
                FacesContext.getCurrentInstance().addMessage(findComponent(context.getViewRoot(), "form").getClientId(), new FacesMessage(FacesMessage.SEVERITY_ERROR, "No tiene permisos para realizar ésta acción", "No tiene permisos para realizar ésta acción"));                return null;
            }
