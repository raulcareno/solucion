...
//        }
        ...
        if (!permisos.verificarPermisoReporte("Institucion", "ingresar_institucion.jspx", "ingresar", true, "ADMINISTRACION")) {
            try {
...
//                Logger.getLogger(InstitucionBean.class.getName()).log(Level.SEVERE, null, ex);
    ...
    public String guardar() {
        ...
        if (object.getIdInstitucion() == 0) {
            if (!permisos.verificarPermisoReporte("Institucion", "agregar_institucion.jspx", "agregar", true, "ADMINISTRACION")) {
            ...
            if (!permisos.verificarPermisoReporte("Institucion", "actualizar_institucion.jspx", "agregar", true, "ADMINISTRACION")) {
                FacesContext.getCurrentInstance().addMessage(findComponent(context.getViewRoot(), "form").getClientId(), new FacesMessage(FacesMessage.SEVERITY_ERROR, "No tiene permisos para realizar ésta acción", "No tiene permisos para realizar ésta acción"));                return null;
            }
            try {
    ...
    public String eliminar(Institucion obj) {
        FacesContext context = FacesContext.getCurrentInstance();
        try {
            if (!permisos.verificarPermisoReporte("Institucion", "eliminar_institucion.jspx", "eliminar", true, "ADMINISTRACION")) {
                FacesContext.getCurrentInstance().addMessage(findComponent(context.getViewRoot(), "form").getClientId(), new FacesMessage(FacesMessage.SEVERITY_ERROR, "No tiene permisos para realizar ésta acción", "No tiene permisos para realizar ésta acción"));                return null;
            }
