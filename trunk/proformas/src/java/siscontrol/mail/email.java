...
//                    "RECUPERACION DE CLAVE " + emp.getNombres() , empre.getUsuariomail(), empre.getClavemail(), empre.getSmtp(), empre.getPuerto(), empre.getAutorizacion(), empre.getStar());
//            if (estado) {
...
public void llamarNuevo(){
    ...
    public String soporteTecnico(String cedula,ArrayList proforma,ArrayList para,String mensaj,String tema) {
        ...
        if (emp != null) {
            ...
            Boolean estado = EnviarAutenticacion.RecuperarClave(para, mensaje,
                    tema, empre.getUsuariomail(), empre.getClavemail(), 
                    empre.getSmtp(), empre.getPuerto(), empre.getAutorizacion(), empre.getStar(),proforma);
            if (estado) {
                return "[OK] " + emp.getEmail();
            } else {
                return "no";
...
//            
            ...
            Message msg = getTraerMensaje(session, tema, emailsA, mensaje, emailInstitucion, emailInstitucion,archivo);
            Transport.send(msg);
            return true;
        } catch (Exception e) {
...
//        String to = email;
        ...
        try {
            ...
            System.out.println("***********************" + emailInstitucion);
            Message msg = getTraerMensaje(session, tema, emailsA, mensaje, emailInstitucion, respuestaA,archivo);
